...
В секции `apps` вы можете указать несколько конфигураций, это достаточно универсальная возможность. Подходит как для работы с большой кодовой базой и создания разных приложений на одной площадке, так и для работы с сервер-рендером, когда для одного приложения используются разные точки входа.
